 ...
 */
@Controller
public class MessageHandler320106 {
    private static Logger log = Logger.getLogger(MessageHandler320106.class);
    @Autowired
    TcpServerDelegate tcpServerDelegate;

    public String message320106(String msg) {
        long startTime = System.currentTimeMillis();
        log.info("==============message320106开始时间=====================" + startTime);
        //首先解析msg 对msg进行透析
        ...
        try {
            ...
            for (int i = 0; i < subCount; i++) {
                String NRecordSingle = NRecord.substring(i * 340, (i + 1) * 340);
...
//            //个性化信息
                reslutDataSingle.setPlivatemsg(NRecordSingle.substring(124, 324));
                //卡号
                reslutDataSingle.setCardno(NRecordSingle.substring(124, 140));
                //应收金额
                reslutDataSingle.setRecvamount(NRecordSingle.substring(140, 148));
                reslutDataSingle.setAmtneedrecv(NRecordSingle.substring(140, 148));
                //实收金额
                reslutDataSingle.setAmtrecv(NRecordSingle.substring(148, 156));
                //优惠类型1
                reslutDataSingle.setDistypeone(NRecordSingle.substring(156, 158));
                //优惠金额1
                reslutDataSingle.setDistypeoneamt(NRecordSingle.substring(158, 166));
                //优惠类型2
                reslutDataSingle.setDistypetwo(NRecordSingle.substring(166, 168));
                //优惠金额2
                reslutDataSingle.setDistypetwoamt(NRecordSingle.substring(168, 176));
                //优惠类型3
                reslutDataSingle.setDistypethree(NRecordSingle.substring(176, 178));
                //优惠金额3
                reslutDataSingle.setDistypethreeamt(NRecordSingle.substring(178, 186));
                //商户代码
                reslutDataSingle.setUnitid(NRecordSingle.substring(186, 198));
                //账户号
                reslutDataSingle.setAccountid(NRecordSingle.substring(198, 218));
                //交易批次号，没有时填0
                reslutDataSingle.setBatchid(NRecordSingle.substring(218, 226));
                //交易主机流水号
                reslutDataSingle.setHostseq(NRecordSingle.substring(226, 234));
                //设备代码
                reslutDataSingle.setDevid(NRecordSingle.substring(234, 274));
                //预留
                reslutDataSingle.setResv(NRecordSingle.substring(274,324));
                //应收金额
                reslutDataSingle.setAmtreceivable(NRecordSingle.substring(324,332));
                //用户折扣
                reslutDataSingle.setUserdiscount(NRecordSingle.substring(332, 336));
                //结算折扣
                reslutDataSingle.setSettldiscount(NRecordSingle.substring(336, 340));
            ...
            log.info("=======320106接口调用外围系统返回===========" + returnParameter.getCode());
...
//                crdmReturnMsg +
            ...
            log.error("320106DDPException===============" + ex.getCode());
            ex.printStackTrace();
            toReturn = MerdeviceConstants.SIGN_OUT_RESULT +
                    //版本号
                    reVer +
            ...
            log.error("320106Exception===============" + ex.getMessage());
            ex.printStackTrace();
            toReturn = MerdeviceConstants.SIGN_OUT_RESULT +
                    //版本号
                    reVer +
        ...
        log.info("==============message320106耗费时间=====================" + (System.currentTimeMillis() - startTime));
        return newStirng + toReturn;
    }
    
    public static void main(String[] args) {
		String ss = "";
	
		MessageHandler320106 message320106 = new MessageHandler320106();
		message320106.message320106(ss);
    }
}
