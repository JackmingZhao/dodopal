...
<head>
    ...
    <script type="text/javascript" src="../js/crdSysOrder/crdConsumeOrder.js"></script>
...
<form id="listForm" action="exportCrdConsumeOrder" method="post">  
    <table id="crdSysOrderQueryCondition" class="viewtable">
        <tr>
            <th>卡服务订单号:</th>
            <td><input type="text" id="crdOrderNum" name="crdOrderNum" style="width:150px;"></td>
            <th>产品库主订单号:</th>
            <td><input type="text" id="proOrderNum" name="proOrderNum" style="width:150px;"></td>
            <th>POS号:</th>
            ...
            <td>
                ...
                <a href="#" class="easyui-linkbutton" iconCls="icon-eraser"
                   onclick="clearCrdAllText();">重置</a>
            </td>
        </tr>
        <tr>
            <th>卡号:</th>
            <td><input type="text" id="checkCardNo" name="checkCardNo" style="width:150px;"></td>
         	 <th>卡类型:</th>
            <td>
            <select id="cardType" name="cardType" class="easyui-combobox" editable="false" style="width:110px;">
           	</select>
            </td>
            <th>交易金额:</th>
			<td>
					<input type="text"  id="txnAmtStart" name="txnAmtStart" style="width:80px;"/>
					   ~
	               	<input type="text"  id="txnAmtEnd" name="txnAmtEnd"  style="width:80px;"/>
			</td>
        </tr>
         <tr>
         	<th>订单状态:</th>
            <td>
                <select id="crdOrderStates" name="crdOrderStates" class="easyui-combobox" editable="false">
                </select>
            </td>
             <th>订单前状态:</th>
            <td>
          	...
          	<th>交易日期:</th>
			<td>
					<input type="text"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:$('#afterProceRateDateEnd').val()});" id="afterProceRateDateStart" name="afterProceRateDateStart" style="width:80px;" />
					   ~
	               	<input type="text"   onclick="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:$('#afterProceRateDateStart').val()});" id="afterProceRateDateEnd" name="afterProceRateDateEnd" style="width:80px;" />
			</td>
        </tr>
    </table>
    </form>
</div>
<div region="center" border="false" ><table id="crdConsumeOrderTbl" fit="true"></table></div>
<div id="crdSysOrderTblPagination"></div>
<div id="crdConsumeOrderTblToolbar" class="tableToolbar">
<@sec.accessControl permission="crdSys.crdConsumeOrder.view">
    <a href="#" class="easyui-linkbutton" iconCls="icon-view" plain="true" id="queryVerified"
       onclick="findCrdSysOrderByCode()">查看</a>
</@sec.accessControl>
<@sec.accessControl permission="crdSys.crdConsumeOrder.export">
<!--     <a href="#" class="easyui-linkbutton" iconCls="icon-import" plain="true" id="exportVerified"
       onclick="exportVerified()">导出Excel</a> -->
...
<div id="viewCrdSysOrderDialogToolbar" style="text-align:right;">
    <a href="#" style="margin-right:10px;" class="easyui-linkbutton" iconCls="icon-cancel" plain="true"
       onclick="closeViewCrdConsumeOrder()">返回</a>
</div>
<div id="viewCrdSysOrderDialog" style="align:center;" class="easyui-dialog" closable="false" title="" closed="true" draggable="false" maximized="true" toolbar="#viewCrdSysOrderDialogToolbar">
    <fieldset>
        ...
        <table class="viewOnly">
            ...
            <tr>
                <th>消费卡号:</th>
                <td id="consumeCardNoView"></td>
                <th>消费设备号:</th>
                <td id="consumeCardPosCodeView"></td>
            </tr>
            
               <tr>
                <th>消费请求时间:</th>
                <td id="consumeSendCardDateView"></td>
                <th>消费应答时间:</th>
                <td id="consumeResCardDateView"></td>
            </tr>
               <tr>
                <th>结果请求时间:</th>
