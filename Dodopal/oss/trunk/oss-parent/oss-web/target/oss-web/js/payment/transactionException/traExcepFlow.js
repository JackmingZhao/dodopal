...
function initTraFlowTbl(size) {
	var option = {
		...
		colNames : [ 'id', '交易流水号', '订单号', '商户名|用户名', '业务类型','交易类型', '应付金额（元）','实付金额（元）', '外部交易状态', '内部交易状态', '创建时间' ],
		colModel : [ {name : 'id',hidden : true,frozen:true}, 
		             ...
		             {name : 'merOrUserName',index : 'merOrUserName',width : 150,align : 'left',sortable : false, formatter: htmlFormatter},
...
//	var businessTypeQuery = $('#businessTypeQuery').combobox('getValue');
	var tranOutStatusQuery ='3';
	var tranInStatusQuery = '11';
	var businessTypeQuery = '99';
	var realMinTranMoneyQuery = $('#realMinTranMoneyQuery').val();
	var realMaxTranMoneyQuery = $('#realMaxTranMoneyQuery').val();
	if (realMaxTranMoneyQuery != '' && realMaxTranMoneyQuery != ''  && parseFloat(realMinTranMoneyQuery) > parseFloat(realMaxTranMoneyQuery)) {
		$('#realMinTranMoneyQuery').val(realMaxTranMoneyQuery);
		$('#realMaxTranMoneyQuery').val(realMinTranMoneyQuery);
	}
	var traQuery = {
...
function loadViewTraFlow(data) {
	if (data.code == "000000") {
		...
		$('#orderDateView').html(ddpDateFormatter(TraTransactionBean.orderDate));
		...
		$('#finishDateView').html(ddpDateFormatter(TraTransactionBean.createDate));
	} else {
		msgShow(systemWarnLabel, data.message, 'warning');
	}
}
...
function handleException() {
	...
	var selrow = $("#traFlowTbl").getGridParam('selrow');
	if (selrow) {
		$.messager.confirm("确定",'您确定处理吗?',function(r){  
		    if (r){
				var rowData = $('#traFlowTbl').getRowData(selrow);
				ddpAjaxCall({
					url : "exceptionHandle",
					data : rowData.tranCode,
					successFn : function (data) {
			            if (data.code == "000000") {
			            	findTraFlow();
			            } else {
...
/*************************************************  数据导出  *****************************************************************/
var expConfInfo = {
	"btnId"			: "exportExceptionTra", 					/*must*/// the id of export btn in ftl 
	"typeSelStr"	: "PayTraTransactionDTO", 	/*must*/// type of export
	"toUrl"			: "exportExceptionTransaction", 			/*must*/// the export url
	"parDiaHeight"	: "410"
};
var filterConStr = [	// filter the result by query condition
		{'name':'tranOutStatus', 	'value': "3"	},	// eval
		{'name':'tranInStatus',	'value': "11"  },
