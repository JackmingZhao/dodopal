		...
		<input type="hidden" value="${(merchant.id)!}" id="id" />
			...
			<table  class="base-table base-table01">
				...
				<tr>
					...
					<th>账户类型：</th>
					<td>
					<!--TODO  账户${(merchant.merPropertyView)!}-->
				...
				<tr>
					<th>详细地址：</th>
					<td style="white-space:pre">${(merchant.merProName)!}<#if merchant.merCityName??> ${(merchant.merCityName)!}市 <#else></#if>${(merchant.merAdds)!}</td>
					<th>通卡公司：</th>
					<td>
					<#if (merchant.merBusRateBeanList?size>0)>
						...
						title="<#list merchant.merBusRateBeanList as merBusRate>${(merBusRate.proName)!} </#list>">
			...
			<table class="base-table base-table01">
				...
				<tr>
					...
					<td>
						...
						<#if merchant.merBankNameView??>
							<#switch merchant.merBankNameView>
           					<#case "1">
           						工商银行
              				<#break>
              				<#case "2">
					...
					<tr>
						...
						<td>
							<select class="com-input-txt w260" id="merBankName" name="merBankName" >
								<option value ="" <#if  merchant.merBankNameView==""> selected </#if>>－－ 请选择 －－</option>
								<option value="1" <#if  merchant.merBankNameView=="1"> selected </#if>>工商银行</option>
								<option value="2" <#if  merchant.merBankNameView=="2"> selected </#if>>交通银行</option>
								<option value="3" <#if  merchant.merBankNameView=="3"> selected </#if>>建设银行</option>
								<option value="4" <#if  merchant.merBankNameView=="4"> selected </#if>>农业银行</option>
								<option value="5" <#if  merchant.merBankNameView=="5"> selected </#if>>中国银行</option>
							</select>
