...
public class MerchantBean extends BaseEntity{
    private static final long serialVersionUID = -123509948926660244L;
    /*商户编码*/
    ...
    /*当前商户功能合集*/
    private List<MerFunctionInfoBean> merFunInfoList;

    public List<MerFunctionInfoBean> getMerFunInfoList() {
        return merFunInfoList;
    }

    public void setMerFunInfoList(List<MerFunctionInfoBean> merFunInfoList) {
...
//	}
    ...
    public String getProviderNameView(){
		...
		if (null != arrayBean && arrayBean.length > 0) {
			for (MerBusRateBean bean : arrayBean) {
				if(proNameMap.containsKey(bean.getProName())) {
					continue;
				}
				str.append("，" + bean.getProName());
				proNameMap.put(bean.getProName(), null);
			}
		}
		if(str.length()>0) {
    ...
    /**
     * Title:门户系统子商户列表展示
     * Time:2015-09-09 18:51
     * Name:Joe
     */
    public String getMerStateView() {
        if (StringUtils.isBlank(this.merState)) {
            return null;
        }
       return MerStateEnum.getMerStateByCode(this.merState).getName();
    }
}
