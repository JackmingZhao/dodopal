...
public class SupplierSecureController extends CommonController{
    private static Logger log = Logger.getLogger(AccountSecureController.class);
    @Autowired
    ...
    /******************************************门户系统安全设置********************************/
    @RequestMapping("/showSupplierSecure")
     public ModelAndView verifiedMgmt(HttpServletRequest request) {
         return new ModelAndView("prvd/supplierSecure");
     }
   ...
   @RequestMapping("/saveAccountMoblie")
    public @ResponseBody DodopalResponse<String> saveAccountMoblie(HttpServletRequest request,@RequestBody MobileBean mobileBean){
        ...
        try {
            ...
            merUserBean.setId(super.getCurrentUserId(session));
            ...
            if (ResponseCode.SUCCESS.equals(response.getCode())) {
                PortalUserDTO portalUser = (PortalUserDTO) session.getAttribute(PortalConstants.SESSION_USER);
                portalUser.setMerUserMobile(mobileBean.getMoblieNum());
                session.setAttribute(PortalConstants.SESSION_USER, portalUser);
            }
        } catch (Exception e) {
