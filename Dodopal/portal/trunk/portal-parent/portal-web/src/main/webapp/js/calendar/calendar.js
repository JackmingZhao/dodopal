...
Calendar.prototype.toString = function()
{
	...
	str +="<iframe frameborder='0' id='backgroundShim' src='about:blank'  style='position:absolute;height:100%;width:100%;left:0; z-index:-1'></iframe></div>";
	return str;
};
Calendar.prototype.getStyle = function()
{
	var str = '<style type="text/css">\n';
	str += '.calendar{position:absolute;width:170px!important;width /**/:170px;height:175px!important;height /**/:170px;background-color:'+this.style.bodyBackColor+';border:1px solid ' + this.style.borderColor + ';left:0px;top:0px;z-index:9999;margin:5px 0 0 0;}\n';
	str += '.cdrHeader{vertical-align:middle;background-color:'+ this.style.headerBackColor +';width:170px;height:24px;line-height: 18px;font-size:12px;color:'+this.style.headerFontColor+';}\n';
	str += '.cdrWatermark{position:absolute;left:0px;top:60px;width:170px;font-family: Arial Black;font-size:100px;line-height:78px;color:'+this.style.watermarkColor+';z-index:1;text-align:center;}\n';
	str += '.cdrBodyBar{background-color:' + this.style.bodyBarBackColor + ';font-size:12px;color:' + this.style.bodyBarFontColor + ';width:170px;height:20px;line-height: 20px;}\n';
	str += '.cdrBody{width:170px;height:100px!important; height /**/:100px;font-size:12px;cursor:pointer;color:' + this.style.bodyFontColor + ';}\n';
	str += '.dayOver{padding:0px;border:none;background-color:#37b4e9;color:#ffffff;line-height:17px;}\n';
	str += '.dayOut{padding:1px;border:none;height:16px;line-height: 17px;}\n';
...
{
	var str = '<table Author="alin" class="cdrHeader" cellSpacing="2" cellPadding="0"><tr Author="alin" align="center" style="vertical-align:middle;">\n';
	str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="previousYear" title="prev_year" style="cursor:pointer;width:10px;" onclick="'+this.Obj+'.onChangeYear(false);"><<</td>\n';
	str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="previousMonth" title="prev_month" style="cursor:pointer;width:10px;" onclick="'+this.Obj+'.onChangeMonth(false);"><</td>\n';
	...
	str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="nextMonth" title="next_month" style="cursor:pointer;width:10px;" onclick="'+this.Obj+'.onChangeMonth(true);">></td>\n';
	str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="nextYear" title="next_year" style="cursor:pointer;width:10px;" onclick="'+this.Obj+'.onChangeYear(true);">>></td></tr>\n';
	str += '</table>\n';
	return str;
};
...
Calendar.prototype.getYearMenu = function(year)
{
	...
	for(i = 0; i < 10; i=i+2)
	{
		...
		var _ndate = new Date(_nyear,this.date.getMonth(),this.date.getDate());
		str += '<tr Author="alin" align="center">';
		
		str += '<td Author="alin" width="13%" height="16" ';
		...
		str += '<td Author="alin" width="13%" height="16" ';
		if(this.date.getFullYear() != _nyear)
		{
			str += 'onmouseover="this.className=\'menuOver\'" onmouseout="this.className=\'\'" ';
		}
		else
		...
		str += 'onclick="' + this.Obj + '.bindDate(\'' + _ndate.toFormatString("-") + '\')">' + _nyear + '</td>\n';
		
		
		str += '</tr>';
	}
	str += '<tr Author="alin" align="center"><td Author="alin" colspan="2"><table Author="alin" style="font-size:12px;width:100%;" cellSpacing="0" cellPadding="0">\n';
...
Calendar.prototype.getMonthMenu = function()
{
	...
	for(i = 1; i <= 12; i=i+2)
	{
		...
		var _ndate = new Date(this.date.getFullYear(),i,this.date.getDate());
		
		
		str += '</tr><tr Author="alin" align="center">';
		
		
		
		str += '<td Author="alin" height="16" ';
		...
		str += 'onclick="' + this.Obj + '.bindDate(\'' + _date.toFormatString("-") + '\')">'+i+'月</td>\n';
		
		
		var nextmonth=i+1;
		str += '<td Author="alin" height="16" ';
		if(this.date.getMonth() + 1 != nextmonth)
		{
			str += 'onmouseover="this.className=\'menuOver\'" onmouseout="this.className=\'\'" ';
		}
		else
		...
		str += 'onclick="' + this.Obj + '.bindDate(\'' + _ndate.toFormatString("-") + '\')">'+nextmonth+'月</td>\n';
...
Calendar.prototype.bindDate = function(date)
{
	...
	for(i = 0; i < 40; i++)
	{
		...
		else if(i >= new Date(_year,_month,1).getDay() && _day <= _monthDays[_month])
		{
			...
			var tmpDate=_curDate.toFormatString("yyyy-mm-dd");
			var ss=tmpDate.split('-')
			var sYear=ss[0];
			var sMonth=ss[1];
			var sDay=ss[2];
			...
			if(sMonth<10){
				if(sDay<10){
					_dayElement.value =sYear+"-0"+sMonth+"-0"+sDay;
				}else{
					_dayElement.value =sYear+"-0"+sMonth+"-"+sDay;
				}
			}else{
				if(sDay<10){
					_dayElement.value =sYear+"-"+sMonth+"-0"+sDay;
				}else{
					_dayElement.value =sYear+"-"+sMonth+"-"+sDay;
				}
			}
			_day++;
		}
...
Calendar.prototype.bindHeader = function()
{
	...
	_watermark.innerHTML = this.date.getMonth() + 1;
};
...
{
	if(null!=this.timeOver){
		if(obj.innerHTML != "") {
			var _objVal = new Date(obj.value.replace(/\-/g, "\/"));
			if(_objVal < this.timeOver){
...
Calendar.prototype.showMenu = function(isyear)
{
	...
	if(isyear != null)
	{
		...
		_menu.style.left = (_obj.offsetLeft - 20) + 'px';
		_menu.style.width = /*_obj.offsetWidth + */'94px';
	}
...
var closeCalendar = function (evt){
...
$(document).on("click", function(evt) {
	var _target= evt.target || evt.srcElement;
	var author = $(evt.target).attr('Author');
	
	if(!$(evt.target).attr('Author') &&  _target != inputObj && _target != targetObj)
	{
		getObjById("Calendar").style.display = "none";
	}
});

Calendar.prototype.clearRadio = function(){
	$('#timeRadio input').removeAttr('checked');  
};
