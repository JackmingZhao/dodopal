$(function(){
	...
	$('.page-navi').paginator({prefix : 'order',pageSizeOnChange : findRechargeOrderCountList});
...
function findRechargeOrderCount(page){
	...
	ddpAjaxCall({
		url : $.base+"/businessOrder/findCardRechargeOrderSum",
		data : orderQuery,
...
function findRechargeOrderCountList(page) {
	if (!page) {
		page = {
			pageNo : 1,
			pageSize : 10
	...
	ddpAjaxCall({
		url : $.base + "/businessOrder/findCardRechargeOrderPage",
		data : query,
		successFn : function(data) {
			if (data.code == "000000") {
				var html = '';
				...
				if (data.responseEntity.records
					...
					$(data.responseEntity.records).each(
							function(i, v) {
								...
								html += '<td>'
								html += v.txnAmt == null ? '': v.txnAmt;// 充值金额
								html += '</td>';
								...
								html += '<td>'
								html += v.merGain == null ? '': v.merGain;// 利润
								html += '</td>';
