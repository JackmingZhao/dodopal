$(function() {
	...
	$('#threshold').focusout(function(){
		//$('#thresholdERR').hide();
		validateChildThreshold(true);
	});
	
	//额度阀值
	$('#limit').focusout(function(){
		//$('#limitERR').hide();
		validateChildLimit(true);
	});
...
//收入
function validateIncome(state){
	return $.warnHandler(state,false,$.moneyPointTwo,"income","incomeVal", "数字，整数6位，两位小数");
}


//额度阀值
function validateChildThreshold(state){
	return $.warnHandler(state,true,$.moneyPointTwo,"threshold","thresholdERR", "数字，整数6位，两位小数");
}


//额度
function validateChildLimit(state){
	return $.warnHandler(state,true,$.moneyPointTwo,"limit","limitERR", "数字，整数6位，两位小数");
}
