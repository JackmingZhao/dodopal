$(function(){
	...
	addFocusEvent();
	createAreaSelectWithSelectId("provinceG","cityG");
});

function addMerchantUser(){
	resetDetailForm('detailForm');
	setTitleForAction('#merchantUserDetail .tit-h3','创建新用户');
	createAreaSelectWithSelectId("provinceG","cityG"); //初始化省份和城市
	$('#addressG').val('');//初始化详细地址
	$('#merUserRemark').val('');//备注
	$('#merchantUserId').val('');
...
function loadMerchantUser(data,viewOnly){
	...
	if(null!=data.income){
		data.income = Number(data.income/100).toFixed(2);
	}
	
	if(viewOnly){
		data.isMarried = data.isMarriedView;
		data.education = data.educationView;
		if(data.income!=null){
			data.income = data.income+"元";			
		}else{
			data.income = "";
		}
	...
	$('#isMarried').val(data.isMarried);
	$('#select_isMarried').attr('disabled',true);
	$('#education').val(data.education);
	$('#birthday').val(data.birthday);
	//省市
	if(data.merUserPro!=null){
		//省市
		setProvinceCity('provinceG','cityG',data.merUserPro,data.merUserCity);
		$('#editProCityLine').show();
		$('#addProCityLine').hide();
		$('#provinceView').html(data.merUserProName);
		$('#cityView').html(data.merUserCityName);
	}
	
	//地址
	$('#addressG').val(data.merUserAdds);
	selectUiInit();
...
function getFormValues(){
	var inputIncome = null;
	if($('#income').val()!=""){
		inputIncome = Number($('#income').val())*100;
	}
	...
	var merchantUser = {
			...
			merUserPro :provinceG,
			merUserCity:cityG,
			merUserAdds:addressG,
			merGroupDeptList: getSelectedDepartment()
	}
	return merchantUser;
}
function checkProvinceG() {
	var provinceG = getProvinceValue('provinceG');
	if (isNotBlank(provinceG)) {
		$("#addProCityVal").html('');
	}
}
function checkAddressG() {
		var addressG = $('#addressG').val();
		if (isNotBlank(addressG)) {
			$("#addressGVal").html('');
}
}
