...
function selectedCheckbox() {
	merNameS = $('#merName').val().split(',');

	for (var i = 0; i < merNameS.length; i++) {

		$("table :checkbox").each(function(key, value) {
			if ($(value).prop('name') == merNameS[i]) {
...
function initDiscount() {
	$("#discountEdit").bind('keyup', function() {
		clearNoNum($(this));
	});
	$("#discountEdit").bind('keydown', function() {
		checkDecimal($(this), 1, 9, 0, 1);
	});
	$("#discountEdit").bind('blur', function() {
		clearNoNumOnBlur($(this));
	});
	
	
	$("#sortEdit").bind('keyup', function() {
		clearNoNum1($(this));
	});
}
