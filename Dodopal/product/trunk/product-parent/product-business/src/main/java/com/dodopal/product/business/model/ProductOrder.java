...
public class ProductOrder extends BaseEntity {
    ...
    // 重发标志位（0：生单默认值；1：申请充值密钥环节更新为1——产品库接收到DLL请求时，以此判断是否为重发请求）
    ...
    // 页面显示使用 订单状态。
    private String showProOrderState;
    ...
    public String getShowProOrderState() {
        return showProOrderState;
    }
    ...
    public String getShowProOrderStateView() {
        if (StringUtils.isBlank(this.showProOrderState)) {
            return null;
        }
       return RechargeOrderStatesEnum.getRechargeOrderStatesEnumByCode(this.showProOrderState).getName();
    }
    public void setShowProOrderState(String showProOrderState) {
        this.showProOrderState = showProOrderState;
    }
