...
$(function(){
	ddpAjaxCall({
		url : $.base+"/cardRecharge/getCardRechargeInfo",
		data : cityCode,
...
function toChangeCity(name,code){
	...
	$("#city").html(name);
	cityCode = code;
	window.location.href = "toChangeCity?cityCode="+cityCode+"&bnscode=04";
}
...
function getOtherPayWay(){
	var param = "";
	ddpAjaxCall({
		url : $.base+"/cardRecharge/getMorPayWay",
		data : param,
...
function toSureOrder(toTurnFlag){
	...
	if($("#accountId").is(':checked')){
		...
		if(needBankMoney>0){
			...
			//可以不用needBankMoney 直接拿到选择的产品金额，看是否选择了账户充值，如果没有则判断是否选择了银行
...
function createOrder(){
	...
	ddpAjaxCall({
		url : $.base+"/loadOrder/personBookLoadOrder",
		data : orderRequest,
...
function bindCard(){
	...
	var map = {
	 ...
	 ddpAjaxCall({
			url : $.base+"/cardRecharge/toBindCard",
			data : map,
			async:false,
			successFn : function(data){
				$("#bindCardDiv").hide();
...
function closeWebPage(){
	...
	function cardNoBlur(){
		$('#cardNo').blur(function(){
			...
			if(carNo == ""){
				$("#cardNoErr").show();
				$("#cardNoErr").html("请输入您的一卡通卡号");
			}else if(!$.enNoMax($.trim(carNo))){
				$("#cardNoErr").show();
				$("#cardNoErr").html("只能输入英文及数字");
			}else{
				$("#cardNoErr").hide();
			}
		});
		$('#cardNoAga').blur(function(){
			checkCardNo();
		});
	}
	...
	function checkCardNo(){
		var carNo = $('#cardNo').val();
		var carNoTwo = $('#cardNoAga').val();
		cardCode = $('#cardNoAga').val();
		if(carNo == ""){
			$("#cardNoErr").show();
			$("#cardNoErr").html("请输入您的一卡通卡号");
		}else if(!$.enNoMax($.trim(carNo))){
			...
			$("#cardNoAgaErr").show();
		}else if(carNo != carNoTwo){
			$("#cardNoAgaErr").html("两次输入的卡号不一致！");
			$("#cardNoAgaErr").show();
		}else{
			$("#cardNoErr").hide();
			$("#cardNoAgaErr").hide();
