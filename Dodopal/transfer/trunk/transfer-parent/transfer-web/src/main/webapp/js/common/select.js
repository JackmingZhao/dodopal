...
function rSelects() {
	...
	for (i=0;i<selects.length;i++){
		//select_info_merBankName
		if(""==selects[i].name){
			continue;
		}
		removeNode('select_' + selects[i].name);
...
function clickSelects(name){
	...
	if(getBrowserInfo()=="7.0"){
		getID(sincn).setAttribute("myNum","true");
		bodyclick.onclick = function(){
			for (i=0;i<selects.length;i++){	
				if(""==selects[i].name){
					continue;
				}
				if(getID('select_info_' + selects[i].name).getAttribute("myNum")=="true"){
					getID('select_info_' + selects[i].name).setAttribute("myNum","false");
					continue;
				}
				getID('select_info_' + selects[i].name).className = 'tag_select';
				getID('options_' + selects[i].name).style.display = 'none';
...
function clickOptions(i, n, name){		
	...
	var value = getID(name).options[getID(name).selectedIndex].value; 
	selects[i].options[n].selected = 'selected';
	var sNewValue = getID(name).options[getID(name).selectedIndex].value;
	if(sNewValue!=value){
		if(getID(name).getAttribute("onchange")!=null){
			if(getBrowserInfo()=="7.0"){
				getID(name).getAttribute("onchange")();
			}else{
				eval(getID(name).getAttribute("onchange"));	
			}
		}
	}
	if(getID(name).getAttribute("city")!=null){
		changeCity(name,getID(name).getAttribute("city"));
	}
	
}

function setOption(id, n, name){		
	var li = getID('options_' + name).getElementsByTagName('li');
	n = getID(id).selectedIndex;
	...
	getID( 'options_' + name ).style.display = 'none' ;
	getID( 'select_info_' + name ).className = 'tag_select';
	getID(id).options[n].selected = 'selected';
